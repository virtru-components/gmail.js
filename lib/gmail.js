  ...
  api.tools.parse_email_data = function(email_data) {
    ...
    for(i in email_data) {
      ...
      if(x[0] == 'ms') {
        ...
        if (x[13] != undefined) {
          var temp = x[13];
          data.threads[x[1]].content_html = temp[6];
          data.threads[x[1]].to = temp[1];
          data.threads[x[1]].cc = temp[2];
          data.threads[x[1]].bcc = temp[3];
          data.threads[x[1]].attachments = temp[7] != undefined ? temp[7][0] : undefined;
        } else {
          data.threads[x[1]].content_html = x[8];
        }
      }
    }
